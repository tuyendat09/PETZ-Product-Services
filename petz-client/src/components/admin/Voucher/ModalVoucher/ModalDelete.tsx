...
export default function ModalDelete({
  ...
  const [hiddenVoucher, { data, error: hiddenError }] =
    useToggleVoucherMutation();

  async function handleDeleteCategory() {
    if (voucherId) {
      const toggleVoucherObject = { voucherId: voucherId, toggleOption: true };
      await hiddenVoucher(toggleVoucherObject as any);
    }
  }
  ...
  return (
    ...
    >
      <ModalContent className="dark:text-white">
        {(onClose) => (
          <>
            <ModalHeader className="text-center">Ẩn voucher</ModalHeader>
            <ModalBody>
              <p>Bạn có chắc chắn muốn ẩn voucher</p>
            </ModalBody>
            <ModalFooter>
              <Button
